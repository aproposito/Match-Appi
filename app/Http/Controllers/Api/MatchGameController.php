...
{
    private const WITH_TEAMS = ['homeTeam', 'awayTeam'];
    #[OA\Get(
        path: '/api/matches',
    ...
    {
        if ($request->user()->role === 'admin' || $request->query('scope') === 'all') {
            $matches = MatchGame::with(self::WITH_TEAMS)->get();
        } else {
            $matches = MatchGame::with(self::WITH_TEAMS)
                ->whereBetween('match_date_time', [
                    now('Europe/Madrid')->startOfDay()->setTimezone('UTC'),
                    now('Europe/Madrid')->addDay()->setTime(8, 0, 0)->setTimezone('UTC'),
                ])
                ->get();
        }
    ...
    public function update(UpdateMatchRequest $request, MatchGame $matchGame)
    {
        ...
        $matchGame->load(self::WITH_TEAMS);
